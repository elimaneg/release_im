...
mvn_release() {
    ...
    if [ "${MVN_DEBUG_RELEASE}" = "true" ] ; then
       ...
       #exit  
       ${MAVEN} $MVN_ARGS ${MVN_RELEASE_PREPARE_ARGS} -B release:prepare && ${MAVEN} $MVN_ARGS ${MVN_RELEASE_PERFORM_ARGS} release:perform
    else
...
get_dev_version(){
 if $(${GIT} rev-parse 2>/dev/null); then
    ...
    echo "WORKING_DIR================= $WORKING_DIR" >> /tmp/jaja
    cd $WORKING_DIR
    checkout_branch ${DEV}
